...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!$complain_id) {
            throw new Exception('Invalid input: Missing ID');
        }
        ...
        // Update data
        $query = $obj->updateData("tbl_complain_templates", $form_data, $where_cond);

        // Return success response
        if ($query === true) {
            echo json_encode([
                'success' => true,
                'message' => 'complain template deleted successfully'
            ]);
        } else {
