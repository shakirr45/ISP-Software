...
$PreviousYear = $obj->rawSql("SELECT count(ag_id), MONTH(entry_date) AS month 
                              FROM tbl_agent WHERE YEAR(entry_date)='$previousYear' AND ag_status='0' GROUP BY MONTH(entry_date)");

$CurrentYear = $obj->rawSql("SELECT count(ag_id), MONTH(entry_date) AS month 
                             FROM tbl_agent WHERE YEAR(entry_date)='$currentYear' AND ag_status='0' GROUP BY MONTH(entry_date)");
...
foreach ($PreviousYear as $data) {
    $previousYearData[$data['month'] - 1] = $data["count(ag_id)"];
}
foreach ($CurrentYear as $data) {
    $currentYearData[$data['month'] - 1] = $data["count(ag_id)"];
}
