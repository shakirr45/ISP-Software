...
// Logging function
function logMessageInterface($msg){
...
function snmpWalkLines($community, $oltIp, $oid){
    ...
    logMessageInterface("Executing: $command");

    $output = shell_exec($command);

    if(empty($output)){
        logMessageInterface("ERROR: Empty SNMP response for OID: $oid");
        return [];
    }

    if(strpos($output, 'Timeout') !== false || strpos($output, 'No response') !== false){
        logMessageInterface("ERROR: SNMP timeout for OID: $oid");
        return [];
    }

    if(strpos($output, 'No Such Object') !== false){
        logMessageInterface("WARNING: OID not found: $oid");
        return [];
    }

    logMessageInterface("SNMP response length: " . strlen($output) . " chars");
    return explode("\n", trim($output));
}
...
logMessageInterface("Starting interface status fetch from OLT: $oltIp");
...
foreach($oids as $key => $oid){
    logMessageInterface("Fetching $key...");
            ...
            switch($key){
                ...
                case 'onu_status':
                    if($type === 'INTEGER'){
                        ...
                        logMessageInterface("$key for index $index: $statusCode -> " . $data[$index][$key]);
                    }
                    break;
...
logMessageInterface("Found " . count($onuPorts) . " EPON interfaces");

if(empty($onuPorts)){
    logMessageInterface("WARNING: No EPON interfaces found!");
    exit;
}
...
try {
    ...
    foreach($onuPorts as $onu){
        try {
            ...
            logMessageInterface("Error processing " . ($onu['interface_name'] ?? 'unknown') . ": " . $e->getMessage());
            $errors++;
        }
    }

    $pdo->commit();
    logMessageInterface("SUCCESS: Database updated. Inserted: $inserted, Updated: $updated, Errors: $errors");

} catch(Exception $e) {
    $pdo->rollBack();
    logMessageInterface("CRITICAL ERROR: Database transaction failed: " . $e->getMessage());
    exit(1);
}

logMessageInterface("Interface status data fetch completed successfully.");
?>
