...
$PreviousYear = $obj->rawSql("SELECT SUM(acc_amount) as amount , MONTH(entry_date) as month FROM tbl_account WHERE  YEAR(entry_date)='$previousYear' AND acc_type='4' GROUP BY MONTH(entry_date)");
$CurrentYear = $obj->rawSql("SELECT SUM(acc_amount) as amount , MONTH(entry_date) as month FROM tbl_account WHERE  YEAR(entry_date)='$currentYear'  AND acc_type='4' GROUP BY MONTH(entry_date)");
