...
if(isset($_POST['submit'])){
    ...
    if(isset($_POST['account_id']) && !empty($_POST['account_id'])){
        ...
        echo ' <script>window.location="?page=view_all_income"; </script>';exit;
    } 
...
if (isset($_POST['search'])) {
    ...
    $expenseDetails = $obj->rawSql("SELECT * FROM tbl_account WHERE entry_date BETWEEN '$dateform' AND '$dateto' AND acc_type = '2' ORDER BY entry_date DESC");
} else {

    $dateform = $firsDayOfMonth->format('Y-m-d');
    $dateto = date('Y-m-d');
    $expenseDetails = $obj->rawSql("SELECT * FROM tbl_account WHERE MONTH(entry_date) = MONTH(CURRENT_DATE) AND YEAR(entry_date) = YEAR(CURRENT_DATE) AND acc_type = '2' ORDER BY entry_date DESC;");
